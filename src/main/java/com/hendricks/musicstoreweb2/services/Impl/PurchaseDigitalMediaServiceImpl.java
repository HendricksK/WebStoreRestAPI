 ...
 */
@Service
public class PurchaseDigitalMediaServiceImpl implements PurchaseDigitalMediaService{
    @Autowired
    ...
    switch (type) {
       ...
       ProductPurchase pc2 = repo.findOne(id);
            for(int x = 0; pc2.getDgPurchases().size() > x; x++){
                if(pc2.getDgPurchases().get(x).getName().equals(name)){
                    return pc2.getDgPurchases().get(x).getPrice();
                }
            }
       return null;    
    }
